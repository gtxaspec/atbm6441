...
{
	int ret = 0;
	struct wakeup_event_req wk_event;
	...
	{
		wk_event.event = WAKEUP_TCP_NETPATTERN;
	}
	else if (strcmp(argv[0], "UDP_PATTERN") == 0)
	{
		wk_event.event = WAKEUP_UDP_NETPATTERN;
	}
	else if (strcmp(argv[0], "WK_SSID") == 0)
	{
		wk_event.event = WAKEUP_WK_SSID;
	}
	else if (strcmp(argv[0], "MAGIC_PKT") == 0)
	{
		wk_event.event = WAKEUP_MAGIC_PKT;
	}
