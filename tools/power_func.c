...
static int __init atbm_module_init(void)
{
	...
	s_atbm_info.power_gpio = 38;//GPIOG(17);
...
//	sw_gpio_setcfg(s_atbm_info.power_gpio, GPIO_CFG_INPUT);
	...
	s_atbm_info.power_irq_num = request_irq(gpio_to_irq(s_atbm_info.power_gpio), power_gpio_irq, IRQF_TRIGGER_RISING| IRQF_TRIGGER_FALLING| IRQF_DISABLED ,   "power_gpio_irq", NULL);
//	s_atbm_info.power_irq_num = sw_gpio_irq_request(s_atbm_info.power_gpio, TRIG_EDGE_DOUBLE, power_gpio_irq, NULL);
	...
	s_atbm_info.insmod_gpio = 39;//GPIOG(18);
...
//	sw_gpio_setcfg(s_atbm_info.insmod_gpio, GPIO_CFG_INPUT);
	gpio_direction_input(s_atbm_info.insmod_gpio);
//	request_irq(gpio_to_irq(host->pdata->gpio->cd.num), jzmmc_detect_interrupt,IRQF_TRIGGER_RISING| IRQF_TRIGGER_FALLING| IRQF_DISABLED ,   "mmc-insert-detect", host);
	s_atbm_info.insmod_irq_num = request_irq(gpio_to_irq(s_atbm_info.insmod_gpio), insmod_gpio_irq, IRQF_TRIGGER_RISING| IRQF_TRIGGER_FALLING| IRQF_DISABLED ,   "insmod_gpio_irq", NULL);
...
//	sw_gpio_irq_free(s_atbm_info.power_irq_num);
	gpio_free(s_atbm_info.power_irq_num);
err_power_irq:
...
{
	gpio_free(s_atbm_info.insmod_irq_num);
	gpio_free(s_atbm_info.insmod_gpio);
	gpio_free(s_atbm_info.power_irq_num);
	gpio_free(s_atbm_info.power_gpio);
